...
for i in range(20):
    ...
    F = []
    sums = []
    F_data_points, sums_data = variations(50,F,alpha,sums)
    mu_F, sigma_F = data_analysis(F_data_points)
    mu_sum, sigma_sum = data_analysis(sums_data)
    mu.append(mu_F)
    sigma.append(sigma_F)
    


plt.errorbar(t, mu, sigma, linestyle='-', marker='.', capsize=3, color='purple')
plt.xlabel('alpha')
plt.ylabel('Percentage difference in companies failed')
#plt.xlim(0,105)
#plt.ylim(0,0.025)
plt.show()
